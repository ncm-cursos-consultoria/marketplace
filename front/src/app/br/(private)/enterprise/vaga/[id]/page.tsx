...
// Dicionário de estilo para os badges de status (com tipagem corrigida)
...
function JobCard({ job }: { job: ApiJob }) {
  ...
  return (
    <Link
      href={jobDetailUrl}
      className="block bg-white border rounded-lg shadow-sm p-4 transition-all hover:shadow-md hover:border-blue-300 cursor-pointer"
    >
      <div className="flex justify-between items-start">
        ...
        <span className={`ml-4 flex-shrink-0 inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${statusStyles[job.status as JobStatus]}`}>
          {statusTranslations[job.status] || job.status}
        </span>
      </div>

      {job.tags && job.tags.length > 0 && (
        <div className="flex items-center flex-wrap gap-2 pt-3 mt-3 border-t border-gray-100">
